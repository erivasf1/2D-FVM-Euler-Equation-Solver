...
//  array<double,3>* &field;
  ...
  void OutputPrimitiveVariables_VTS(string filename,vector<array<double,4>>* &field,MeshGenBASE* &mesh); //outputs primitive variables in .vts format for ParaView visualization

  void OutputManufacturedSourceTerms(string filename,vector<array<double,4>>* &field,MeshGenBASE* &mesh); //outputs primitive variables in .vts format for ParaView visualization
