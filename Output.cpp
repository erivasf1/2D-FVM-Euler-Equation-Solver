...
void Output::CalculateOrderofAccuracy(const char *filename_read,const char *filename_write){
  ...
  for (int n=0;n<(int)CellSize.size()-1;n++){ 
    PHat_density[n] = (log(Density[n+1]) / log(Density[n])) / log(r);
    PHat_velocity[n] = (log(Velocity[n+1]) / log(Velocity[n])) / log(r);
    PHat_pressure[n] = (log(Pressure[n+1]) / log(Pressure[n])) / log(r);
 }
